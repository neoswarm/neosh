...
pub struct Host {
    ...
    /// What the bus was last told about focus and mode, so the next batch says only what moved.
...
impl Host {
    pub fn new(
        ...
        editor.set_home(Some(composer_win));
        ...
        match out {
            ...
            ScriptOutbound::Plugin { plugin, msg } => match msg {
                ...
                PluginOutbound::Call { id, call } => {
                    ...
                    // Answered when the command's owner answers. Waiting for that on the loop
                    // would freeze typing for as long as the handler takes, and a handler is
                    // allowed to open a picker and wait for a person.
    ...
    fn say_what_moved(&mut self, batch: &[UiEvent]) {
        let mut said: Vec<(&'static str, serde_json::Value)> = Vec::new();
        for ev in batch {
            match ev {
                UiEvent::FocusChanged { win } => {
                    if *win == self.last_focus {
                        continue;
                    }
                    if let Some(from) = self.last_focus.take() {
        ...
        if mode != self.last_mode {
            self.last_mode = mode;
            said.push(("neosh.mode", serde_json::json!({ "mode": mode })));
        }
